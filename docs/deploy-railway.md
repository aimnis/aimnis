...
| `AIMNIS_DEMO_API_KEY` | `aim_…` | Optional **deliberately-public** try-out key, pre-filled in the MCP server card's config form so directory visitors (e.g. Smithery) can try the server without registering. Use a dedicated portal-issued key (standard metered caps bound abuse; revoke by prefix to rotate) — never an admin key. Unset ⇒ visitors are prompted for their own key. |
